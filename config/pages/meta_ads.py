...
Exibe análise detalhada de campanhas do Meta Ads (Facebook e Instagram).
...
def render(project_name: str):
    """Renderiza a página de Meta Ads."""
    ...
    if not config.Meta_Token:
        st.warning("⚠️ Token do Meta Ads não configurado. Vá em '🔌 Conexões' para configurar.")
        return
    ...
    # --- GERAR DADOS SIMULADOS DE ANÚNCIOS ---
    np.random.seed(42)
    num_ads = 15
    ...
    # --- KPIs ---
    st.subheader("📊 KPIs - Meta Ads")
    
    kpi_col1, kpi_col2, kpi_col3, kpi_col4 = st.columns(4)
    
    with kpi_col1:
        st.metric("💰 Gasto Total", f"R$ {df_ads['Gasto'].sum():,.2f}")
    
    with kpi_col2:
        st.metric("📈 Faturamento", f"R$ {df_ads['Faturamento'].sum():,.2f}")
    
    with kpi_col3:
        st.metric("🎯 Leads", f"{df_ads['Leads'].sum():.0f}")
    
    with kpi_col4:
        st.metric("💹 ROI Médio", f"{df_ads['ROI'].mean():.2f}%")
    ...
    with col2:
        # Gráfico de Barras: Top 10 Anúncios por Faturamento
        top_ads = df_ads.nlargest(10, "Faturamento")
        fig_bars = px.bar(
            top_ads,
            x="Faturamento",
            y="Nome",
            orientation="h",
            title="Top 10 Anúncios por Faturamento",
            color="ROI",
            color_continuous_scale="RdYlGn"
        )
    ...
    # --- TABELA DE ANÚNCIOS ---
    st.subheader("📋 Anúncios Detalhados")
    
    # Formatação para exibição
    df_display = df_ads.copy()
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"meta_ads_{start_date}_{end_date}.csv",
        mime="text/csv"
    )
