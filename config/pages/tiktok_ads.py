#!/usr/bin/env python
"""
Página de TikTok Ads

Exibe análise detalhada de campanhas do TikTok Ads.
...
def render(project_name: str):
    """Renderiza a página de TikTok Ads."""
    ...
    if not config.TikTok_Token:
        st.warning("⚠️ Token do TikTok Ads não configurado. Vá em '🔌 Conexões' para configurar.")
        return
    ...
    with col1:
        date_range = st.date_input(
            ...
            format="DD/MM/YYYY",
            key="tiktok_date_range"
        )
    
    with col2:
        if st.button("🔄 Atualizar", key="tiktok_refresh"):
            st.cache_data.clear()
            st.rerun()
    ...
    with col1:
        ...
        fig_scatter = px.scatter(
            ...
            size="Gasto",
            color="ROI",
            hover_data=["Nome", "Objetivo", "Gasto", "Leads"],
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"tiktok_ads_{start_date}_{end_date}.csv",
        mime="text/csv"
    )
